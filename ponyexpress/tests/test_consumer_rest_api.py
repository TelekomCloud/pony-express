...
class BasicTestCaseV1(TestServerBase):
            ...
            'packages': [
                {
                    ...
                    "architecture": "amd64",
                    "id": "5d8e40ce35ea30f621573d40b17dcd21e3b974f2dd5e096c6c10701af8cdc5d0",
                    "provider": "apt",
                    "archive": "precise"
                }
            ]
    ...
    def testRequestNodePackages(self):
        ...
        eq_(package['id'], self.DATA['node2']['packages'][0]['id'])
    ...
    def testRequestPackageInfo(self):
        self.addNode(self.DATA['node2'])
        p = self.DATA['node2']['packages'][0]
        j = self.get_json('/v1/package/' + p['id'])
        eq_(type(j), dict)
        eq_(j["name"], p["name"])
        eq_(j["uri"], p["uri"])
